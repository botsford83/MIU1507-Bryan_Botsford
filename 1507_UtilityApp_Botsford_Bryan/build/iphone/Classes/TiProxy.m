...
//Rare exceptions to indicate a bug in the _k1507_utilityapp_botsford_bryan code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_k1507_utilityapp_botsford_bryan = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_k1507_utilityapp_botsford_bryan]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.k1507_utilityapp_botsford_bryan.module"
