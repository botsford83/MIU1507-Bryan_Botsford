...
if [ "x$TITANIUM_CLI_XCODEBUILD" == "x" ]; then
    /usr/local/bin/node "/Users/bryanbotsford1/.appcelerator/install/4.0.0/package/node_modules/appc-cli-titanium/node_modules/titanium/bin/titanium" build --platform iphone --sdk "4.0.0.GA" --no-prompt --no-progress-bars --no-banner --no-colors --build-only --xcode
    exit $?
else
