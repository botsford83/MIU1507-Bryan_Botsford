...
//Rare exceptions to indicate a bug in the _miu1507_botsford_bryan_informationapp code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_miu1507_botsford_bryan_informationapp = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_miu1507_botsford_bryan_informationapp]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.miu1507_botsford_bryan_informationapp.module"
