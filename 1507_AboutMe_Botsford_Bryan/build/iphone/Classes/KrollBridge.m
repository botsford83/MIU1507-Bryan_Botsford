...
NSString * k1507_AboutMe_Botsford_Bryan$ModuleRequireFormat = @"(function(exports){"
...
@implementation k1507_AboutMe_Botsford_BryanObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k1507_aboutme_botsford_bryan);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_k1507_aboutme_botsford_bryan gc];
}
...
{
	// create k1507_AboutMe_Botsford_Bryan global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k1507_AboutMe_Botsford_Bryan" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k1507_aboutme_botsford_bryan = [[k1507_AboutMe_Botsford_BryanObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k1507_aboutme_botsford_bryan];
	
	NSString *_k1507_aboutme_botsford_bryanNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k1507_aboutme_botsford_bryanNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k1507_aboutme_botsford_bryan valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k1507_AboutMe_Botsford_Bryan$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k1507_aboutme_botsford_bryan.kroll" 
