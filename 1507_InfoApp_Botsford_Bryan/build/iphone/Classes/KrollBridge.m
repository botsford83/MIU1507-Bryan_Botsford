...
NSString * MIU1507_Botsford_Bryan_InformationApp$ModuleRequireFormat = @"(function(exports){"
...
@implementation MIU1507_Botsford_Bryan_InformationAppObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_miu1507_botsford_bryan_informationapp);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_miu1507_botsford_bryan_informationapp gc];
}
...
{
	// create MIU1507_Botsford_Bryan_InformationApp global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "MIU1507_Botsford_Bryan_InformationApp" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_miu1507_botsford_bryan_informationapp = [[MIU1507_Botsford_Bryan_InformationAppObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_miu1507_botsford_bryan_informationapp];
	
	NSString *_miu1507_botsford_bryan_informationappNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _miu1507_botsford_bryan_informationappNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_miu1507_botsford_bryan_informationapp valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:MIU1507_Botsford_Bryan_InformationApp$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.miu1507_botsford_bryan_informationapp.kroll" 
