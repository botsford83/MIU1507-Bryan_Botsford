...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kMIU1507_Botsford_Bryan_InformationAppUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kMIU1507_Botsford_Bryan_InformationAppUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a MIU1507_Botsford_Bryan_InformationApp app using the node.js CLI
